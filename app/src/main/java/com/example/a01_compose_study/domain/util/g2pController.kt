...
class G2PController @Inject constructor(val viewModel: PttViewModel) : IG2PListener {
    ...
    override fun onG2PStatusChanged(mode: Int, state: Int, deviceId: String) {
        ...
        ServiceState.systemState.g2pStatus[g2pMode]?.value = g2pState

        when (g2pState) {
            HG2PStatus.SUCCESS,
            HG2PStatus.FAILED,
            HG2PStatus.CANCELLED -> {
                ServiceState.isWaitingPBG2PState.value = false
            }
    ...
    override fun onG2PCompleted(mode: Int, deviceId: String, count: Int) {
        ...
        ServiceState.systemState.g2pCompleteCnt[g2pMode]?.value = count
    }
